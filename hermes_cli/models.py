...
_openrouter_catalog_cache_scope_fp: str | None = None
_openrouter_policy_catalog_cache: dict[str, dict[str, Any]] | None = None
_openrouter_policy_catalog_cache_at: float = 0.0
_openrouter_policy_catalog_cache_scope_fp: str | None = None
_OPENROUTER_USER_MODELS_URL = "https://openrouter.ai/api/v1/models/user?limit=500"
_OPENROUTER_CATALOG_CACHE_TTL = 300.0


def _openrouter_policy_scope_fingerprint(api_key: str, endpoint: str) -> str:
    """Return a non-reversible cache partition for an account and endpoint."""
    scope = f"{endpoint.strip().rstrip('/').lower()}\0{api_key}".encode(
        "utf-8", errors="replace"
    )
    return hashlib.blake2b(scope, digest_size=16).hexdigest()
...
def _fetch_openrouter_policy_catalog(
    ...
    *,
    ...
    """Fetch the authenticated catalog filtered by account privacy policy.

    ``None`` means eligibility could not be verified. An empty dict is a valid
    response meaning that the account currently has no eligible models.
    """
    ...
    if (
        ...
        and _openrouter_policy_catalog_cache_scope_fp == scope_fp
        and not force_refresh
        and (now - _openrouter_policy_catalog_cache_at)
        < _OPENROUTER_CATALOG_CACHE_TTL
    ):
    ...
    _openrouter_policy_catalog_cache_scope_fp = scope_fp
    return dict(catalog)
...
def _openrouter_policy_model_ids(
    *,
    ...
    """Return models eligible under the active OpenRouter account policy."""
    catalog = _fetch_openrouter_policy_catalog(
        force_refresh=force_refresh, api_key=api_key
    )
    if catalog is None:
        return None
    if tool_capable:
...
def fetch_openrouter_models(
    ...
    *,
    ...
    """Return all tool-capable models allowed by the account policy.

    Curated models are ordered first, followed by every other eligible model.
    The picker fails closed when the authenticated catalog cannot be verified,
    except that a stale cache from the exact same credential/endpoint scope may
    be used during a transient outage.
    """
    ...
    if (
        ...
        and _openrouter_catalog_cache_scope_fp == scope_fp
        and not force_refresh
    ...
    # snapshot when the manifest is unreachable. This determines ordering only;
    # eligibility always comes from the authenticated account catalog.
    try:
    ...
    if live_by_id is None:
        if _openrouter_catalog_cache_scope_fp == scope_fp:
            return list(_openrouter_catalog_cache or [])
        return []

    ordered_ids = list(preferred_ids)
    seen_ids = set(ordered_ids)
    ordered_ids.extend(model_id for model_id in live_by_id if model_id not in seen_ids)
    ...
    _openrouter_catalog_cache_scope_fp = scope_fp
    return list(models)
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # Provider plugins may be routable without an auth.PROVIDER_REGISTRY row.
    # Include their declared credentials so account-scoped catalogs cannot be
    # reused after a key rotation.
    try:
        from providers import get_provider_profile
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    try:
        ...
        if provider is None or normalized == "openrouter":
            ...
            _openrouter_catalog_cache_scope_fp = None
            _openrouter_policy_catalog_cache = None
            _openrouter_policy_catalog_cache_at = 0.0
            _openrouter_policy_catalog_cache_scope_fp = None
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "openrouter":
        ...
        if eligible_models is None:
            return {
                ...
                "message": (
                    ...
                    "The model was not selected; check the OpenRouter credential and retry."
                ),
            }
        if requested_for_lookup in set(eligible_models):
            return {
                "accepted": True,
                "persist": True,
                "recognized": True,
        ...
        suggestion_text = ""
        if suggestions:
            suggestion_text = " Similar policy-eligible models: " + ", ".join(
                f"`{model_id}`" for model_id in suggestions
            )
        return {
            "accepted": False,
            "persist": False,
